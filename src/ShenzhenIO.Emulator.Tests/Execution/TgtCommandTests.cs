...
{
    public class TgtCommandTests
    {
        [Theory]
        [InlineData(1, 0, TestResult.Success)]
        [InlineData(0, 1, TestResult.Failure)]
        [InlineData(1, 1, TestResult.Failure)]
        [InlineData(-100, 100, TestResult.Failure)]
        [InlineData(100, -100, TestResult.Success)]
        [InlineData(-100, -100, TestResult.Failure)]
        [InlineData(100, 100, TestResult.Failure)]
        public void TestCheckingValueGreatnessImmediately(int first, int second, TestResult expectedTestResult)
        {
            // Arrange
            ...
            var command = new TgtCommand(firstValueSource, secondValueSource);
